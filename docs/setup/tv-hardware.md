...
Samsung TVs that are not hospitality models (This includes their Professional TVs) manufactured after 2022 feature a device discovery service that cannot be disabled and can cause issues and are generally not recommended.
...
- Can we control the TV and Apple TV using [CEC](https://en.wikipedia.org/wiki/Consumer_Electronics_Control) Which allows the Apple TV to be placed anywhere or hidden completely.
- If CEC does not work does the remote control everything using Infra Red?
- Are there any obvious issues with the TV that would cause issues for guests?

!!! info
    If CEC is not a reliable option, the Apple TV will need to placed in a location where the remote control can send it commands. This will need to be considered as part of the room design. 

### Steps to verify TV compatibility

1. Check the TV model number against our [list of tested TVs](tv-compat-matrix.md).
